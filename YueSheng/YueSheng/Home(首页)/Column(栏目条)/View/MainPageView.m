...
@interface MainPageView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
+(instancetype)mainPageView{
    ...
    return [[NSBundle mainBundle] loadNibNamed:@"MainPageView" owner:nil options:nil].lastObject;
...
-(void)awakeFromNib{
    ...
    self.columnView.backgroundColor = YWColorRGBA(0, 0, 0, 0.05);
     ...
     [self.pageView registerNib:[UINib nibWithNibName:@"MainPageViewCell" bundle:nil] forCellWithReuseIdentifier:pageID];
    self.pageView.tag = 2;
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (collectionView.tag == 1) {
      ...
      MainPageViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:pageID forIndexPath:indexPath];
        
        if (self.pageStatusArray.count) {
        YWPageStatus *pageStatus = self.pageStatusArray[indexPath.item];
